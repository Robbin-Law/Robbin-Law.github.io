...
![](img/PN-Junction-Block-Diagram.JPG)
...
What current would the Ideal Model predict? ‌ ‌μA

What current would the Practical Model predict? ‌  ‌μA
...
What current would the Ideal Model predict? ‌  mA

What current would the Practical Model predict? ‌  mA
...
What does the Practical Model say the voltage drop across the diode would be? ‌  V

Using the Complete Model, determine the current in this circuit. ‌  mA

 Given that the Complete Model includes the Barrier Potential and the drop across the internal resistance in the diode's voltage drop, what is the voltage drop across the diode using this model? ‌  V

What is the error introduced by using the Practical Model in this case? ‌  %
...
If a diode with a reverse leakage current of 500 nA is reverse biased in a series circuit with an 8.2 M‌Ω resistor, powered by a 12 V supply, what voltage would the Practical Model predict across the diode? ‌  V

What voltage would the Complete Model predict across the diode? ‌  V
...
VD is 0.0 V, since the closed switch is a short
I = VR/R = (6 - 0)/22 k‌Ω = 273 ‌μA
Practical Model:

VD = 0.7 V
I = VR/R = (6 - 0.7)/22 k‌Ω = 241 ‌μA
Complete Model:

We'll have to start with the current, using KVL:  I = (6.0 - 0.7)/(22 k‌Ω + 5.9 ‌Ω) = 241 ‌μA
VD = 0.7 + I * r'D = 0.701 V
...
VD is 0.0 V, since the closed switch is a short
I = VR/R = (6 - 0)/120 ‌Ω = 50 mA
Practical Model:

VD = 0.7 V
I = VR/R = (6 - 0.7)/120 ‌Ω = 44.2 mA
Complete Model:

We'll have to start with the current, using KVL:  I = (6.0 - 0.7)/(120 ‌Ω + 5.9 ‌Ω) = 42.1 mA
VD = 0.7 + I * r'D = 0.948 V
...
VD is 0.0 V, since the closed switch is a short
I = VR/R = (90 - 0)/56 k‌Ω = 1.61 mA
Practical Model:

VD = 0.7 V
I = VR/R = (90 - 0.7)/56 k‌Ω = 1.59 mA
Complete Model:

We'll have to start with the current, using KVL:  I = (90 - 0.7)/(56 k‌Ω + 5.9 ‌Ω) = 1.59 mA
VD = 0.7 + I * r'D = 0.709 V
...
VD = 12 V, since there is no current through the resistor, and therefore no voltage drop from the supply.
Practical Model:
...
I = 500 nA
VD = 12 - I * R = 12 - 500 nA * 56 k‌Ω = 11.97 V
...
I = 500 nA
VD = 12 - I * R = 12 - 500 nA * 10 M‌Ω = 7.00 V
...
Notice that we still use Ohm's Law and Kirchhoff's Laws throughout.  All we need to to is replace the diode with the appropriate model, and the basic laws of electricity apply.
...
The voltage at point A can be determined mathematically, as it will be equal to the source voltage minus the voltage drop across the resistor:  VA = VCC - I * R and, since I is zero, VA = 10.0 V - 0*2.7 k‌Ω = 10.0 V.
...
The current through an open circuit is zero.
The voltage at point A can be determined mathematically, as it will be equal to the voltage drop across the resistor:  VA = I * R and, since I is zero, VA = 0*2.7 k‌Ω = 0.0 V.
...
The voltage at A, referenced to ground, is VCC - VD = 12.0 V - 0.7 V = 11.3 V
The current, determined using the resistor, is I = ‌ΔVR/R = (VCC - VD)/R = 11.3 V/ 4.7 k‌Ω = 2.40 mA
...
The voltage at A, referenced to ground, is VD = 0.7 V.
The current, determined using the resistor, is I = ‌ΔVR/R = (VCC - VD)/R = (9.0 V - 0.7 V)/1.8 k‌Ω = 4.61 mA
...
The voltage at A, referenced to ground, is 0.7 V above VCC.  From KVL, VA =VCC + VD = = -10.0 V + 0.7 V = -9.3 V
The current, determined using the resistor, is I = ‌ΔVR/R = (0 - -9.3 V)/1.8 k‌Ω = 5.17 mA
